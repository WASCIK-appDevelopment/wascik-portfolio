...
export async function POST(request: Request) {
  ...
  if (!ownerPhotoUrl || !merchant || !product) {
    return NextResponse.json({ error: "Select a saved owner photo and product/service first." }, { status: 400 });
  }
  ...
  try {
    ...
    const prompt = [
      ...
      productReferenceIncluded
        ? "The SECOND image is the exact advertised product reference. Preserve its recognizable form, proportions, materials and design. Do not replace it with an invented product."
        : `Advertising context: ${merchant} — ${product}. Do not invent logos or readable packaging text.`,
      ...
      creativeMode === "lifestyle" && productReferenceIncluded
        ? "For lifestyle mode, integrate the owner and exact product into a believable real-world use scene. If the product is furniture, seat or position the person naturally on/with it. If it is wearable or handheld, make the physical interaction convincing."
        : "",
      creativeMode === "composite" && productReferenceIncluded
        ? "For composite mode, use sophisticated editorial depth and overlap, but still make the owner and product feel captured in the same visual world rather than pasted together."
        : "",
      ...
      expression && expression !== "Preserve original"
        ? `Expression: ${expression}. Make the facial expression visibly match this direction while preserving identity.`
        : "Preserve the original expression unless another instruction requires a subtle natural change.",
      interaction && interaction !== "Preserve original pose"
        ? `Product interaction / pose: ${interaction}. Make the interaction physically convincing.`
        : "Preserve the original pose only when it still works naturally inside the requested scene.",
    ...
    await recordOpenAIUsage({
      ...
      estimatedCostUsd,
      metadata: { merchant, product, gaze, expression, interaction, productReferenceIncluded, quality, size, creativeMode, refinement, creativeProfile: profile.key },
    });
    ...
    return NextResponse.json({
      ...
      creativeMode,
      refinement,
      mode: "owner-product-cohesive-full-scene-edit",
    });
  } catch (error) {
