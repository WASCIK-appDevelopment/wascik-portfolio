...
function Gauge({ label, percent, amount, detail }: { label: string; percent: number; amount: string; detail: string }) {
  const safePercent = Math.min(100, Math.max(0, Number.isFinite(percent) ? percent : 0));
  const color = gaugeColor(safePercent);
...
export default function OwnerUsageGauges() {
  ...
  return <section style={{ marginBottom: 12, border: "1px solid rgba(89,203,255,.25)", borderRadius: 17, padding: "12px 10px", background: "linear-gradient(135deg,rgba(4,24,39,.96),rgba(3,14,24,.98))", boxShadow: "inset 0 0 24px rgba(52,189,255,.05)" }}>
    <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", gap: 10, marginBottom: 10 }}>
      <div><div style={{ color: "#6bdcff", fontSize: 10, fontWeight: 950, letterSpacing: ".14em" }}>OPENAI USAGE</div><div style={{ marginTop: 2, color: "#91a9b7", fontSize: 10 }}>Green is healthy. At 75% usage the gauges move into orange/red.</div></div>
    ...
    <div style={{ display: "grid", gridTemplateColumns: "repeat(3,minmax(0,1fr))", gap: 8 }}>
      <Gauge label="Leads" percent={Number(data?.leadsUsedPercent || 0)} amount={`$${leads.toFixed(3)} used`} detail={`of $${budget.toFixed(2)} working budget`} />
      <Gauge label="Ads" percent={Number(data?.adsUsedPercent || 0)} amount={`$${ads.toFixed(3)} used`} detail={`copy + photo generation`} />
      <Gauge label="Overall" percent={Number(data?.budgetUsedPercent || 0)} amount={`$${remaining.toFixed(2)} left`} detail={`$${total.toFixed(3)} OpenAI spend`} />
    </div>
