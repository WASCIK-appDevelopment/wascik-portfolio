...
export default function ProductHealthMonitor() {
  ...
  async function repairImages() {
    ...
    try {
      ...
      const data = await response.json().catch(() => ({}));
      if (!response.ok) throw new Error(data.error || "Could not repair missing thumbnails.");
      setImageRepairs([]); setNotice(data.message || "Missing thumbnails repaired.");
    } catch (reason) { setError(reason instanceof Error ? reason.message : "Could not repair missing thumbnails."); }
