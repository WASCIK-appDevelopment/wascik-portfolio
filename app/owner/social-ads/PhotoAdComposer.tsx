...
export default function PhotoAdComposer({ product, platform, headline, cta, creativeNotes = "", onImageCost }: Props) {
  ...
  useEffect(() => { void loadPhotoLibrary(); return () => { if (ownerPhotoIsLocal && ownerPhotoUrl) URL.revokeObjectURL(ownerPhotoUrl); }; }, []);
  ...
  async function uploadPhotos(event: ChangeEvent<HTMLInputElement>) {
    ...
    try {
      ...
      await loadPhotoLibrary(); setNotice(`${uploaded} photo${uploaded === 1 ? "" : "s"} saved permanently in My Photos.`);
  ...
  function selectSavedPhoto(photo: SavedPhoto) {
    ...
    setOwnerPhotoUrl(photo.url); setOwnerPhotoName(`${photo.category} · ${photo.label}`); setOwnerPhotoIsLocal(false); setNotice(`${photo.label} selected for this ad.`);
    if (backgroundDataUrl) void compose(backgroundDataUrl, photo.url);
  }

  async function deleteSavedPhoto(photo: SavedPhoto) {
    if (!confirm(`Delete ${photo.label} from My Photos?`)) return;
    ...
    if (!ownerPhotoIsLocal && ownerPhotoUrl === photo.url) { setOwnerPhotoUrl(""); setOwnerPhotoName(""); if (backgroundDataUrl) void compose(backgroundDataUrl, ""); }
    await loadPhotoLibrary(); setNotice(`${photo.label} removed from My Photos.`);
  }
  ...
  async function generatePhotoAd() {
    ...
    try {
      const response = await fetch("/api/owner/social-ads/image", { method: "POST", headers: { "Content-Type": "application/json", "x-wascik-owner-key": key }, body: JSON.stringify({ merchant: product.merchant, product: product.title, category: product.category || "", platform, headline, creativeNotes, quality, layout, style }) });
      ...
      setBackgroundDataUrl(data.imageDataUrl); setModel(data.model || ""); const cost = typeof data.estimatedCostUsd === "number" ? data.estimatedCostUsd : null; setImageCost(cost); if (cost != null && cost > 0) onImageCost?.(cost); await compose(data.imageDataUrl, ownerPhotoUrl);
  ...
  function temporaryPhotoChanged(event: ChangeEvent<HTMLInputElement>) {
    ...
    if (ownerPhotoIsLocal && ownerPhotoUrl) URL.revokeObjectURL(ownerPhotoUrl); const url = URL.createObjectURL(file); setOwnerPhotoUrl(url); setOwnerPhotoName(`Temporary · ${file.name}`); setOwnerPhotoIsLocal(true); if (backgroundDataUrl) void compose(backgroundDataUrl, url);
  }
  function removeOwnerPhoto() { if (ownerPhotoIsLocal && ownerPhotoUrl) URL.revokeObjectURL(ownerPhotoUrl); setOwnerPhotoUrl(""); setOwnerPhotoName(""); setOwnerPhotoIsLocal(false); if (backgroundDataUrl) void compose(backgroundDataUrl, ""); }
  ...
  return <section style={{ display: "grid", gap: 14, border: "1px solid rgba(113,220,255,.24)", borderRadius: 16, padding: 16, background: "rgba(113,220,255,.035)" }}>
    <div><div style={{ color: "#71dcff", fontSize: 12, fontWeight: 900 }}>AI PHOTO AD GENERATOR</div><h2 style={{ margin: "5px 0 0" }}>Create the downloadable picture ad</h2><p style={{ margin: "7px 0 0", color: "#9fb5c5", lineHeight: 1.55 }}>Upload your photos once to My Photos, then tap the exact version you want to use. Saved photos are kept in private storage and can be reused in future ads without another upload.</p></div>
    ...
    <div style={{ border: "1px solid rgba(113,220,255,.24)", borderRadius: 14, padding: 13, background: "rgba(2,12,21,.62)" }}>
      ...
      {photoLibraryOpen ? <div style={{ display: "grid", gap: 12, marginTop: 12 }}>
        ...
        <div style={{ color: "#8fa6b6", fontSize: 11 }}>Choose a category, then select one or several photos from your phone. Maximum 10 MB each.</div>
        {libraryLoading ? <div style={{ color: "#71dcff" }}>Loading My Photos…</div> : null}
        {!libraryLoading && !photos.length ? <div style={{ color: "#9fb5c5", padding: 12, border: "1px dashed rgba(255,255,255,.15)", borderRadius: 12 }}>No saved photos yet. Upload your batch here during testing.</div> : null}
        {photos.length ? <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill,minmax(105px,1fr))", gap: 9, maxHeight: 360, overflowY: "auto" }}>{photos.map(photo => <div key={photo.id} style={{ border: ownerPhotoUrl === photo.url ? "2px solid #71dcff" : "1px solid rgba(255,255,255,.12)", borderRadius: 12, padding: 6, background: ownerPhotoUrl === photo.url ? "rgba(113,220,255,.1)" : "rgba(255,255,255,.03)" }}><button type="button" onClick={() => selectSavedPhoto(photo)} style={{ width: "100%", border: 0, padding: 0, background: "transparent", color: "white", cursor: "pointer", textAlign: "left" }}><img src={photo.url} alt={photo.label} style={{ width: "100%", aspectRatio: "4 / 5", objectFit: "cover", borderRadius: 8, display: "block" }} /><div style={{ marginTop: 5, fontSize: 10, fontWeight: 850, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{photo.label}</div><div style={{ fontSize: 9, color: "#8fa6b6" }}>{photo.category}</div></button><button type="button" onClick={() => void deleteSavedPhoto(photo)} style={{ marginTop: 4, border: 0, background: "transparent", color: "#ff9f9f", padding: 0, fontSize: 9, cursor: "pointer" }}>Delete</button></div>)}</div> : null}
      </div> : null}
    </div>

    <div style={{ border: "1px solid rgba(255,255,255,.1)", borderRadius: 13, padding: 11, background: "rgba(255,255,255,.025)" }}><strong>Photo selected for this ad</strong><div style={{ marginTop: 7, color: ownerPhotoName ? "#8ff0b8" : "#8fa6b6", fontSize: 12 }}>{ownerPhotoName || "None — this will be a product-only ad."}</div><div style={{ display: "flex", gap: 8, flexWrap: "wrap", marginTop: 9 }}><label style={smallButton}>Use One-Time Photo<input type="file" accept="image/*" onChange={temporaryPhotoChanged} style={{ display: "none" }} /></label>{ownerPhotoUrl ? <button type="button" onClick={removeOwnerPhoto} style={{ ...smallButton, color: "#ffaaaa", borderColor: "rgba(255,120,120,.35)" }}>Remove from this ad</button> : null}</div></div>
    ...
    {backgroundDataUrl ? <div style={{ border: "1px solid rgba(255,215,111,.23)", borderRadius: 13, padding: 12, background: "rgba(255,215,111,.045)", color: "#dbe9f1", fontSize: 13 }}><strong style={{ color: "#ffd76f" }}>Image generation cost</strong><div style={{ marginTop: 6 }}>Model: {model || "image model"} · Quality: {quality} · Estimated image cost: {imageCost != null ? `$${imageCost.toFixed(3)}` : "shown in OpenAI account usage"}</div>{ownerPhotoUrl ? <div style={{ marginTop: 5, color: "#8ff0b8" }}>Adding your saved photo uses no additional OpenAI image generation.</div> : null}</div> : null}
  </section>;
}
