...
function wrap(ctx: CanvasRenderingContext2D, text: string, max: number) {
  ...
  for (const word of words) {
    ...
    if (!line || ctx.measureText(test).width <= max) line = test;
...
export default function PhotoAdComposerV3({ product, platform, headline, cta, visualHook = "", visualSupportLine = "", visualCta = "", creativeNotes = "", onImageCost }: Props) {
  ...
  function selectPhoto(photo: SavedPhoto) {
    setOwnerUrl(photo.url);
    setOwnerName(`${photo.category} · ${photo.label}`);
    setEditedOwnerUrl("");
  ...
  async function upload(event: ChangeEvent<HTMLInputElement>) {
    ...
    try {
      ...
      await loadLibrary();
      setLibraryOpen(true);
      setNotice("Photo saved to My Photos. Tap it to use it in this ad.");
    } catch (reason) {
  ...
  async function editOwnerIfNeeded(): Promise<EditedScene | null> {
    ...
    setEditCost(cost);
    if (cost && onImageCost) onImageCost(cost);
  ...
  function addCopyOverlay(ctx: CanvasRenderingContext2D, width: number, height: number) {
    const margin = Math.round(width * .065);
    const top = Math.round(height * .052);
    const textWidth = width - margin * 2;

    const topShade = ctx.createLinearGradient(0, 0, 0, height * .38);
    topShade.addColorStop(0, "rgba(0,0,0,.78)");
    topShade.addColorStop(.55, "rgba(0,0,0,.42)");
    topShade.addColorStop(1, "rgba(0,0,0,0)");
    ctx.fillStyle = topShade;
    ctx.fillRect(0, 0, width, height * .40);

    const bottomShade = ctx.createLinearGradient(0, height * .68, 0, height);
    bottomShade.addColorStop(0, "rgba(0,0,0,0)");
    bottomShade.addColorStop(.50, "rgba(0,0,0,.44)");
    ...
    ctx.font = `800 ${Math.round(width * .020)}px system-ui,sans-serif`;
    ctx.fillText(product.merchant.toUpperCase().slice(0, 40), margin, top);
    ...
    while (hookLines.length > 2 && headlineSize > Math.round(width * .040)) {
      headlineSize -= 3;
      ctx.font = `900 ${headlineSize}px system-ui,sans-serif`;
      hookLines = wrap(ctx, hook, textWidth);
    }
    hookLines = hookLines.slice(0, 2);
    const hookGap = Math.round(headlineSize * 1.05);
    const hookY = top + Math.round(width * .067);
    ctx.fillStyle = "#fff";
    hookLines.forEach((line, i) => ctx.fillText(line, margin, hookY + i * hookGap));

    if (support) {
      ctx.fillStyle = "rgba(255,255,255,.86)";
      ctx.font = `600 ${Math.round(width * .022)}px system-ui,sans-serif`;
      const supportLines = wrap(ctx, support, textWidth).slice(0, 2);
      const supportY = hookY + hookLines.length * hookGap + Math.round(width * .018);
      supportLines.forEach((line, i) => ctx.fillText(line, margin, supportY + i * Math.round(width * .030)));
    }

    const ctaW = width - margin * 2;
    const ctaH = Math.round(height * .058);
    const ctaY = Math.round(height * .835);
    ctx.fillStyle = "rgba(0,0,0,.70)";
    roundRect(ctx, margin, ctaY, ctaW, ctaH, Math.round(ctaH * .18));
    ctx.fill();
    ...
    ctx.fillText(button, width / 2, ctaY + Math.round(ctaH * .66));

    ctx.fillStyle = "rgba(255,255,255,.60)";
    ctx.font = `700 ${Math.round(width * .014)}px system-ui,sans-serif`;
    ctx.fillText(product.merchant === "WASCIK App Development" ? "WASCIK App Development" : "WASCIK Affiliate Services", width / 2, height - Math.round(height * .020));
  }

  async function compose(bgUrl: string, editedScene: EditedScene | null) {
    const canvas = canvasRef.current;
    if (!canvas) return;
    const { width, height } = dims[layout];
    canvas.width = width;
    canvas.height = height;
    const ctx = canvas.getContext("2d");
    if (!ctx) return;

    // The key V3 change: an intelligently edited owner/product scene becomes the
    // entire ad artwork. It is no longer pasted into a smaller rounded rectangle.
    if (editedScene?.url) {
      const scene = await loadImage(editedScene.url);
      cover(ctx, scene, 0, 0, width, height);
    } else {
      const bg = await loadImage(bgUrl);
      cover(ctx, bg, 0, 0, width, height);

      if (product.image_url) {
        const productImage = await loadImage(product.image_url);
        const safeTop = Math.round(height * .27);
        const safeBottom = Math.round(height * .79);
        const boxW = Math.round(width * .72);
        const boxH = safeBottom - safeTop;
        const boxX = Math.round((width - boxW) / 2);
        ctx.save();
        ctx.shadowColor = "rgba(0,0,0,.35)";
        ctx.shadowBlur = Math.round(width * .025);
        ctx.fillStyle = "rgba(255,255,255,.08)";
        roundRect(ctx, boxX, safeTop, boxW, boxH, Math.round(width * .025));
        ctx.fill();
        ctx.restore();
        contain(ctx, productImage, boxX + 28, safeTop + 28, boxW - 56, boxH - 56);
      }
    }
  ...
  async function generate() {
    ...
    setBusy(true);
    setError("");
    setNotice("Building one cohesive intelligent ad scene…");
    try {
      ...
      const response = await fetch("/api/owner/social-ads/image", {
        ...
        body: JSON.stringify({
          ...
          platform,
          headline: hook,
          creativeNotes: [creativeNotes, support ? `Visual support: ${support}` : "", `CTA: ${button}`].filter(Boolean).join("\n"),
          quality,
          layout,
          style,
          creativeMode,
      ...
      const data = await response.json().catch(() => ({})) as ImageResult;
      if (!response.ok || !data.imageDataUrl) throw new Error(data.error || "The ad environment could not be generated.");
      setBackground(data.imageDataUrl);
      setModel(data.model || "");
      const cost = typeof data.estimatedCostUsd === "number" ? data.estimatedCostUsd : null;
      setBackgroundCost(cost);
      if (cost && onImageCost) onImageCost(cost);
      await compose(data.imageDataUrl, editedScene);
      setNotice(`Ad ready · ${profile.label} · full-scene ${creativeMode === "lifestyle" ? "lifestyle integration" : creativeMode === "composite" ? "owner + product composition" : "product campaign"}.`);
    } catch (reason) {
      setError(reason instanceof Error ? reason.message : "The intelligent ad could not be generated.");
    } finally {
      setBusy(false);
    }
  }
  ...
  function refine(next: Refinement) {
    setRefinement(next);
    resetEdit();
    setNotice(`Refinement set to ${next}. Tap Generate Intelligent Photo Ad for a new version.`);
  }
  ...
  return <section className="photoAdV3" style={{ display: "grid", gap: 14, border: "1px solid rgba(113,220,255,.22)", borderRadius: 14, padding: 15, background: "rgba(113,220,255,.025)" }}>
    <div>
      <div style={{ color: "#71dcff", fontSize: 11, fontWeight: 900, letterSpacing: ".1em" }}>INTELLIGENT PHOTO AD · V3</div>
      <h2 style={{ margin: "5px 0 0" }}>Full-scene art director</h2>
      <div style={{ marginTop: 6, color: "#9fb5c5", fontSize: 12 }}>Creative profile: <strong style={{ color: "#dcecf5" }}>{profile.label}</strong> · {profile.mood}</div>
    </div>

    <div className="photoAdV3__grid">
    ...
    <div style={{ borderTop: "1px solid rgba(255,255,255,.08)", paddingTop: 12 }}>
      <div className="photoAdV3__libraryTop"><strong>My Photos</strong><button type="button" onClick={() => setLibraryOpen((v) => !v)} style={buttonStyle}>{libraryOpen ? "Close Library" : "Choose from My Photos"}</button></div>
      {ownerUrl ? <div style={{ display: "grid", gridTemplateColumns: "90px minmax(0,1fr)", gap: 10, marginTop: 10, alignItems: "center" }}><img src={heroOwner} alt="Selected owner" style={{ width: 90, height: 90, objectFit: "cover", borderRadius: 8 }} /><div style={{ minWidth: 0 }}><div style={{ color: "#8ff0b8", fontWeight: 800 }}>Selected for this ad</div><div style={{ fontSize: 11, color: "#9fb5c5", marginTop: 3, overflowWrap: "anywhere" }}>{ownerName}</div><button type="button" onClick={() => { setOwnerUrl(""); setOwnerName(""); resetEdit(); }} style={{ ...buttonStyle, marginTop: 7, color: "#ffaaaa" }}>Remove</button></div></div> : null}
      {libraryOpen ? <div style={{ marginTop: 10, display: "grid", gap: 9 }}><div className="photoAdV3__libraryTop"><select value={photoCategory} onChange={(e) => setPhotoCategory(e.target.value)} style={field}>{categories.map((c) => <option key={c}>{c}</option>)}</select><label style={{ ...buttonStyle, display: "grid", placeItems: "center" }}>{uploading ? "Uploading…" : "Upload from Phone"}<input type="file" multiple accept="image/*" onChange={upload} style={{ display: "none" }} /></label></div><div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill,minmax(min(100px,100%),1fr))", gap: 8, maxHeight: 320, overflowY: "auto" }}>{photos.map((p) => <button key={p.id} type="button" onClick={() => selectPhoto(p)} style={{ border: ownerUrl === p.url ? "2px solid #71dcff" : "1px solid rgba(255,255,255,.1)", borderRadius: 8, padding: 5, background: "rgba(255,255,255,.02)", color: "white", textAlign: "left", minWidth: 0 }}><img src={p.url} alt={p.label} style={{ width: "100%", height: 110, objectFit: "cover", borderRadius: 5 }} /><div style={{ fontSize: 9, marginTop: 4, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{p.label}</div></button>)}</div></div> : null}
    </div>
    ...
    {error ? <div style={{ color: "#ff9f9f", fontSize: 12, overflowWrap: "anywhere" }}>{error}</div> : null}
    {notice ? <div style={{ color: "#8ff0b8", fontSize: 12, overflowWrap: "anywhere" }}>{notice}</div> : null}
