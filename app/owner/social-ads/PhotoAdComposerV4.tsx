...
const dims: Record<Layout, { width: number; height: number; label: string }> = {
  square: { width: 1080, height: 1080, label: "Square · 1:1" }, portrait: { width: 1080, height: 1350, label: "Portrait · 4:5" }, story: { width: 1080, height: 1920, label: "Story / Reel · 9:16" },
};
...
export default function PhotoAdComposerV4({ product, platform, headline, cta, visualHook = "", visualSupportLine = "", visualCta = "", creativeNotes = "", onImageCost }: Props) {
  ...
  const fullDirections = [directorPlan?.sceneBrief, ...(directorPlan?.negativeConstraints || []).map((x) => `Do not: ${x}`), directions, creativeNotes].filter(Boolean).join("\n");
  ...
  async function editOwnerIfNeeded(plan?: DirectorPlan | null): Promise<EditedScene | null> {
    ...
    const response = await fetch("/api/owner/social-ads/owner-edit", { method: "POST", headers: { "Content-Type": "application/json", "x-wascik-owner-key": key() }, body: JSON.stringify({ ownerPhotoUrl: ownerUrl, productImageUrl: product.image_url || "", merchant: product.merchant, product: product.title, category: product.category || "", gaze: plan?.gaze || gaze, expression: plan?.expression || expression, interaction: plan?.interaction || interaction, directions: [plan?.sceneBrief, ...(plan?.negativeConstraints || []).map((x) => `Do not: ${x}`), directions, creativeNotes].filter(Boolean).join("\n"), creativeMode: mode, refinement: plan?.refinement || refinement, identityLock: plan?.identityLock || identityLock, heroPriority: plan?.heroPriority || heroPriority, quality: quality === "high" ? "medium" : "low", layout: plan?.layout || layout }) });
  ...
  function addCopyOverlay(ctx: CanvasRenderingContext2D, width: number, height: number, copyZone: CopyZone = "top") {
    ...
    let headlineSize = Math.round(width * (layout === "story" ? .055 : .050)); ctx.font = `900 ${headlineSize}px system-ui,sans-serif`; let hookLines = wrap(ctx, hook, textWidth); while (hookLines.length > 2 && headlineSize > Math.round(width * .040)) { headlineSize -= 3; ctx.font = `900 ${headlineSize}px system-ui,sans-serif`; hookLines = wrap(ctx, hook, textWidth); } hookLines = hookLines.slice(0, 2); const hookGap = Math.round(headlineSize * 1.05); ctx.textAlign = "left";
    const brandY = copyZone === "top" ? Math.round(height * .050) : Math.round(height * .675); ctx.fillStyle = "rgba(255,255,255,.82)"; ctx.font = `800 ${Math.round(width * .020)}px system-ui,sans-serif`; ctx.fillText(product.merchant.toUpperCase().slice(0, 40), margin, brandY); ctx.fillStyle = "#fff"; ctx.font = `900 ${headlineSize}px system-ui,sans-serif`; const hookY = brandY + Math.round(width * .060); hookLines.forEach((line, i) => ctx.fillText(line, margin, hookY + i * hookGap)); if (support) { ctx.fillStyle = "rgba(255,255,255,.90)"; ctx.font = `600 ${Math.round(width * (copyZone === "top" ? .022 : .020))}px system-ui,sans-serif`; const supportLines = wrap(ctx, support, textWidth).slice(0, copyZone === "top" ? 2 : 1); const supportY = hookY + hookLines.length * hookGap + Math.round(width * .014); supportLines.forEach((line, i) => ctx.fillText(line, margin, supportY + i * Math.round(width * .030))); }
    const ctaW = width - margin * 2; const ctaH = Math.round(height * .058); const ctaY = Math.round(height * .890); ctx.fillStyle = "rgba(0,0,0,.80)"; roundRect(ctx, margin, ctaY, ctaW, ctaH, Math.round(ctaH * .14)); ctx.fill(); ctx.strokeStyle = "rgba(255,255,255,.30)"; ctx.lineWidth = 2; ctx.stroke(); ctx.fillStyle = "#fff"; ctx.textAlign = "center"; ctx.font = `900 ${Math.round(width * .028)}px system-ui,sans-serif`; ctx.fillText(button, width / 2, ctaY + Math.round(ctaH * .66)); ctx.fillStyle = "rgba(255,255,255,.64)"; ctx.font = `700 ${Math.round(width * .014)}px system-ui,sans-serif`; ctx.fillText(product.merchant === "WASCIK App Development" ? "WASCIK App Development" : "WASCIK Affiliate Services", width / 2, height - Math.round(height * .014));
  }
  async function compose(bgUrl: string, editedScene: EditedScene | null) { const canvas = canvasRef.current; if (!canvas) return; const { width, height } = dims[layout]; canvas.width = width; canvas.height = height; const ctx = canvas.getContext("2d"); if (!ctx) return; if (editedScene?.url) { const scene = await loadImage(editedScene.url); cover(ctx, scene, 0, 0, width, height); } else { const bg = await loadImage(bgUrl); cover(ctx, bg, 0, 0, width, height); if (product.image_url) { const productImage = await loadImage(product.image_url); const safeTop = Math.round(height * .25); const safeBottom = Math.round(height * .79); const boxW = heroPriority === "product" ? Math.round(width * .84) : Math.round(width * .72); const boxH = safeBottom - safeTop; const boxX = Math.round((width - boxW) / 2); ctx.save(); ctx.shadowColor = "rgba(0,0,0,.35)"; ctx.shadowBlur = Math.round(width * .025); ctx.fillStyle = "rgba(255,255,255,.08)"; roundRect(ctx, boxX, safeTop, boxW, boxH, Math.round(width * .025)); ctx.fill(); ctx.restore(); contain(ctx, productImage, boxX + 24, safeTop + 24, boxW - 48, boxH - 48); } } addCopyOverlay(ctx, width, height, editedScene?.copyZone || "top"); }

  async function generate() {
    if (busy || planning) return; setBusy(true); setError(""); setNotice("Creative Director is preparing the ad…");
    try {
      let plan = directorPlan;
      if (directorPrompt.trim()) { plan = await askCreativeDirector(); }
  ...
  function refine(next: Refinement) { setRefinement(next); resetEdit(); setNotice(`Refinement set to ${next}. Generate again when ready.`); }
  ...
  return <section className="photoAdV3" style={{ display: "grid", gap: 14, border: "1px solid rgba(113,220,255,.22)", borderRadius: 14, padding: 15, background: "rgba(113,220,255,.025)" }}>
    <div><div style={{ color: "#71dcff", fontSize: 11, fontWeight: 900, letterSpacing: ".1em" }}>INTELLIGENT PHOTO AD · V4</div><h2 style={{ margin: "5px 0 0" }}>Conversational Creative Director</h2><div style={{ marginTop: 6, color: "#9fb5c5", fontSize: 12 }}>Tell the AI what you want naturally. It plans the scene, copy, identity rules, product role, layout and QC before generation.</div></div>
    <div style={{ border: "1px solid rgba(255,215,111,.28)", borderRadius: 12, padding: 12, background: "rgba(255,215,111,.035)", display: "grid", gap: 9 }}><label style={label}><strong style={{ color: "#ffd76f" }}>Tell the Creative Director what you want</strong><textarea value={directorPrompt} onChange={(e) => { setDirectorPrompt(e.target.value); setDirectorPlan(null); resetEdit(); }} rows={5} placeholder="Example: Keep me looking exactly like my original photo. Put the exact product clearly in my hands and make us equal heroes. Use a premium outdoor scene. Do not put text over my face. Put LINK IN BIO at the bottom." style={{ ...field, resize: "vertical" }} /></label><button type="button" disabled={planning || !directorPrompt.trim()} onClick={() => void askCreativeDirector().catch((reason) => { setNotice(""); setError(reason instanceof Error ? reason.message : "The Creative Director could not plan this ad."); })} style={{ ...buttonStyle, color: "#ffd76f", borderColor: "rgba(255,215,111,.42)", background: "rgba(255,215,111,.08)" }}>{planning ? "Creative Director is planning…" : "Plan This Ad"}</button>{directorPlan?.directorSummary ? <div style={{ color: "#dcecf5", fontSize: 12, lineHeight: 1.45 }}><strong>Director plan:</strong> {directorPlan.directorSummary}</div> : null}</div>
    ...
    {creativeMode !== "product" ? <div style={{ display: "grid", gap: 9, borderTop: "1px solid rgba(255,255,255,.08)", paddingTop: 12 }}><strong>Precision Overrides</strong><div className="photoAdV3__directionGrid"><label style={label}>Gaze<select value={gaze} onChange={(e) => { setGaze(e.target.value); resetEdit(); }} style={field}>{gazes.map((x) => <option key={x}>{x}</option>)}</select></label><label style={label}>Expression<select value={expression} onChange={(e) => { setExpression(e.target.value); resetEdit(); }} style={field}>{expressions.map((x) => <option key={x}>{x}</option>)}</select></label><label style={label}>Interaction<select value={interaction} onChange={(e) => { setInteraction(e.target.value); resetEdit(); }} style={field}>{interactions.map((x) => <option key={x}>{x}</option>)}</select></label></div><label style={label}>Extra directions<textarea value={directions} onChange={(e) => { setDirections(e.target.value); resetEdit(); }} rows={3} placeholder="Optional precision override after the Creative Director plan." style={{ ...field, resize: "vertical" }} /></label></div> : null}
    ...
    {background ? <div className="photoAdV3__actions"><button type="button" onClick={() => void saveImage()} style={buttonStyle}>Save Image to iPhone / Photos</button><button type="button" onClick={() => void generate()} style={buttonStyle}>Regenerate From Same Instructions</button></div> : null}
