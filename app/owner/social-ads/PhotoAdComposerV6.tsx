...
const HOUSE_DEFAULT_PROMPT = "Build the strongest polished WASCIK-style social ad for this product or service. Preserve the owner closely when an owner photo is used, make the product or service benefit obvious, keep important text away from the face and product, create a strong headline plus two or three concise benefit callouts, and use a strong platform-appropriate CTA. For Instagram, TikTok, or Threads, default to LINK IN BIO.";
...
type DirectorPlan = { creativeMode?: CreativeMode; identityLock?: IdentityLock; heroPriority?: HeroPriority; gaze?: string; expression?: string; interaction?: string; refinement?: Refinement; style?: Style; layout?: Layout; visualHook?: string; visualSupportLine?: string; visualBenefits?: string[]; visualCta?: string; sceneBrief?: string; negativeConstraints?: string[]; directorSummary?: string };
...
function platformAwareCta(platform: string, visualCta: string, cta: string) { const candidate = compact(visualCta || (cta.length < 28 ? cta : ""), 24).trim(); const bio = /instagram|tiktok|threads/i.test(platform); if (bio) return candidate && !genericCtas.has(candidate.toLowerCase()) ? candidate.toUpperCase() : "LINK IN BIO"; if (!candidate || genericCtas.has(candidate.toLowerCase())) return "SEE DETAILS"; return candidate.toUpperCase(); }
function accentFor(merchant: string, category: string) {
  const key = `${merchant} ${category}`.toLowerCase();
  if (/ticket|concert|event/.test(key)) return "#ffcf57";
  if (/arc.?captain|weld|industrial/.test(key)) return "#ff9d42";
  if (/gearup|gaming/.test(key)) return "#7be7ff";
  if (/aqua|pool|outdoor living/.test(key)) return "#63e6e2";
  if (/coofandy|fashion|apparel/.test(key)) return "#f2d58a";
  if (/eurooptic|hunting|optic|outdoor/.test(key)) return "#b9d58c";
  return "#71dcff";
}
function fallbackBenefits(product: ProductForAd) {
  const key = `${product.merchant} ${product.title} ${product.category || ""}`.toLowerCase();
  if (/ai solutions|artificial intelligence|automation/.test(key)) return ["Automate workflows", "Improve response", "Scale smarter"];
  if (/web development/.test(key)) return ["Modern web presence", "Built for conversion", "Made to grow"];
  if (/app development/.test(key)) return ["Custom app builds", "Modern user experience", "Built around your goals"];
  if (/aqua|chair|lounger|pool/.test(key)) return ["Poolside comfort", "Modern outdoor style", "Relax in style"];
  if (/coofandy|fashion|apparel|shirt|shoe/.test(key)) return ["Modern style", "Everyday versatility", "Standout look"];
  if (/gearup|gaming/.test(key)) return ["Smoother gameplay", "More stable connection", "Less lag disruption"];
  if (/arc.?captain|weld/.test(key)) return ["Shop-ready performance", "Built for demanding work", "Welding gear that works"];
  if (/ticket|concert|event/.test(key)) return ["Concerts & events", "Find seats fast", "More live experiences"];
  if (/optic|binocular|eurooptic|hunting/.test(key)) return ["Outdoor performance", "Precision-focused gear", "Ready for the field"];
  return ["Built for real use", "Clear product value", "See the details"];
}

export default function PhotoAdComposerV6({ product, platform, headline, cta, visualHook = "", visualSupportLine = "", visualCta = "", creativeNotes = "", onImageCost }: Props) {
  ...
  const support = compact(directorPlan?.visualSupportLine || visualSupportLine || "", 92);
  const benefits = (directorPlan?.visualBenefits || fallbackBenefits(product)).map((item) => compact(item, 34)).filter(Boolean).slice(0, 3);
  const button = platformAwareCta(platform, directorPlan?.visualCta || visualCta, cta);
  const accent = accentFor(product.merchant, product.category || "");
  ...
  function applyPlan(plan: DirectorPlan) {
    ...
    setDirectorPlan(plan); resetEdit();
  }
  ...
  async function askCreativeDirector(): Promise<DirectorPlan> {
    ...
    try {
      ...
      const plan = data.plan as DirectorPlan; applyPlan(plan); setNotice(plan.directorSummary || "Creative Director plan applied."); return plan;
    } finally { setPlanning(false); }
  }

  async function editOwnerIfNeeded(plan?: DirectorPlan | null): Promise<EditedScene | null> {
    const mode = plan?.creativeMode || creativeMode; if (!ownerUrl || mode === "product") return null;
  ...
  function addCopyOverlay(ctx: CanvasRenderingContext2D, width: number, height: number, copyZone: CopyZone = "top") {
    ...
    const topShade = ctx.createLinearGradient(0, 0, 0, height * .37); topShade.addColorStop(0, "rgba(0,0,0,.86)"); topShade.addColorStop(.6, "rgba(0,0,0,.38)"); topShade.addColorStop(1, "rgba(0,0,0,0)"); ctx.fillStyle = topShade; ctx.fillRect(0, 0, width, height * .39);
    const bottomShade = ctx.createLinearGradient(0, height * .55, 0, height); bottomShade.addColorStop(0, "rgba(0,0,0,0)"); bottomShade.addColorStop(.5, "rgba(0,0,0,.42)"); bottomShade.addColorStop(1, "rgba(0,0,0,.88)"); ctx.fillStyle = bottomShade; ctx.fillRect(0, height * .53, width, height * .47);

    let headlineSize = Math.round(width * (layout === "story" ? .057 : .052)); ctx.font = `900 ${headlineSize}px system-ui,sans-serif`; let hookLines = wrap(ctx, hook, textWidth); while (hookLines.length > 2 && headlineSize > Math.round(width * .040)) { headlineSize -= 3; ctx.font = `900 ${headlineSize}px system-ui,sans-serif`; hookLines = wrap(ctx, hook, textWidth); }
    hookLines = hookLines.slice(0, 2); const hookGap = Math.round(headlineSize * 1.06); ctx.textAlign = "left";
    const brandY = copyZone === "top" ? Math.round(height * .047) : Math.round(height * .665);

    ctx.fillStyle = accent; roundRect(ctx, margin, brandY - Math.round(width * .024), Math.min(textWidth, Math.round(width * .45)), Math.round(width * .035), Math.round(width * .017)); ctx.fill();
    ctx.fillStyle = "#06131d"; ctx.font = `900 ${Math.round(width * .016)}px system-ui,sans-serif`; ctx.fillText(product.merchant.toUpperCase().slice(0, 34), margin + Math.round(width * .016), brandY);
    ...
    let nextY = hookY + hookLines.length * hookGap + Math.round(width * .010);
    if (support) { ctx.fillStyle = "rgba(255,255,255,.88)"; ctx.font = `650 ${Math.round(width * .020)}px system-ui,sans-serif`; const supportLines = wrap(ctx, support, textWidth).slice(0, 2); supportLines.forEach((line, i) => ctx.fillText(line, margin, nextY + i * Math.round(width * .028))); nextY += supportLines.length * Math.round(width * .030) + Math.round(width * .010); }

    if (copyZone === "top") {
      ctx.font = `800 ${Math.round(width * .016)}px system-ui,sans-serif`;
      let chipX = margin; let chipY = nextY; const gap = Math.round(width * .010); const chipH = Math.round(width * .041);
      for (const benefit of benefits) {
        const chipW = Math.min(textWidth, Math.ceil(ctx.measureText(`✓ ${benefit}`).width + width * .035));
        if (chipX + chipW > width - margin) { chipX = margin; chipY += chipH + gap; }
        ctx.fillStyle = "rgba(3,15,24,.78)"; roundRect(ctx, chipX, chipY - chipH * .72, chipW, chipH, chipH / 2); ctx.fill(); ctx.strokeStyle = accent; ctx.lineWidth = 2; ctx.stroke();
        ctx.fillStyle = accent; ctx.fillText("✓", chipX + Math.round(width * .012), chipY);
        ctx.fillStyle = "#fff"; ctx.fillText(benefit, chipX + Math.round(width * .032), chipY);
        chipX += chipW + gap;
      }
    }

    const socialBio = /instagram|tiktok|threads/i.test(platform);
    if (socialBio) {
      const stripY = Math.round(height * .825); ctx.fillStyle = "rgba(3,15,24,.78)"; roundRect(ctx, margin, stripY, width - margin * 2, Math.round(height * .045), Math.round(height * .018)); ctx.fill();
      ctx.fillStyle = accent; ctx.textAlign = "center"; ctx.font = `900 ${Math.round(width * .021)}px system-ui,sans-serif`; ctx.fillText("➜  LINK IN BIO", width / 2, stripY + Math.round(height * .030));
    }

    const ctaW = width - margin * 2; const ctaH = Math.round(height * .058); const ctaY = Math.round(height * .885); ctx.fillStyle = accent; roundRect(ctx, margin, ctaY, ctaW, ctaH, Math.round(ctaH * .22)); ctx.fill(); ctx.fillStyle = "#06131d"; ctx.textAlign = "center"; ctx.font = `950 ${Math.round(width * .027)}px system-ui,sans-serif`; ctx.fillText(button, width / 2, ctaY + Math.round(ctaH * .66));
    ctx.fillStyle = "rgba(255,255,255,.72)"; ctx.font = `750 ${Math.round(width * .014)}px system-ui,sans-serif`; ctx.fillText(product.merchant === "WASCIK App Development" ? "WASCIK App Development" : "WASCIK Affiliate Services", width / 2, height - Math.round(height * .014));
  }
  ...
  async function generate() {
    if (busy || planning) return; setBusy(true); setError(""); setNotice("Creative Director is preparing the ad…");
    try {
      const plan = await askCreativeDirector(); const mode = plan?.creativeMode || creativeMode;
    ...
    } catch (reason) { setNotice(""); setError(reason instanceof Error ? reason.message : "The intelligent ad could not be generated."); } finally { setBusy(false); }
  }
  ...
  return <section className="photoAdV3" style={{ display: "grid", gap: 14, border: "1px solid rgba(113,220,255,.22)", borderRadius: 14, padding: 15, background: "rgba(113,220,255,.025)" }}>
    <div><div style={{ color: "#71dcff", fontSize: 11, fontWeight: 900, letterSpacing: ".1em" }}>INTELLIGENT PHOTO AD · V6</div><h2 style={{ margin: "5px 0 0" }}>Simple Creative Director</h2><div style={{ marginTop: 6, color: "#9fb5c5", fontSize: 12 }}>Tell the AI what you want in plain English. Leave the box blank to use the richer WASCIK house style automatically.</div></div>
    <div style={{ border: "1px solid rgba(255,215,111,.28)", borderRadius: 12, padding: 12, background: "rgba(255,215,111,.035)", display: "grid", gap: 9 }}><label style={label}><strong style={{ color: "#ffd76f" }}>Tell the AI what you want</strong><textarea value={directorPrompt} onChange={(e) => { setDirectorPrompt(e.target.value); setDirectorPlan(null); resetEdit(); }} rows={5} placeholder="Examples: Put these shoes on me and make them the hero. Make me look at the camera and smile. Leave blank for the normal WASCIK style." style={{ ...field, resize: "vertical" }} /></label><button type="button" disabled={busy || planning || (creativeMode !== "product" && !ownerUrl)} onClick={() => void generate()} style={{ width: "100%", border: 0, borderRadius: 8, padding: "14px 16px", fontWeight: 900, background: busy || planning ? "#314653" : "#71dcff", color: "#031019", fontSize: 16 }}>{busy || planning ? "AI is planning, generating, and checking the ad…" : "Generate Ad"}</button>{directorPlan?.directorSummary ? <div style={{ color: "#dcecf5", fontSize: 12, lineHeight: 1.45 }}><strong>Director plan:</strong> {directorPlan.directorSummary}</div> : null}</div>
    ...
    <details style={{ border: "1px solid rgba(255,255,255,.10)", borderRadius: 10, padding: 10, background: "rgba(255,255,255,.02)" }}><summary style={{ cursor: "pointer", color: "#a9dff2", fontWeight: 900 }}>Advanced Overrides</summary><div style={{ display: "grid", gap: 12, marginTop: 12 }}><div className="photoAdV3__grid"><label style={label}>Ad mode<select value={creativeMode} onChange={(e) => { setCreativeMode(e.target.value as CreativeMode); resetEdit(); }} style={field}><option value="product">Product Only</option><option value="composite">Product + Owner</option><option value="lifestyle">Lifestyle Integration</option></select></label><label style={label}>Quality<select value={quality} onChange={(e) => setQuality(e.target.value as Quality)} style={field}><option value="low">Economy</option><option value="medium">Standard</option><option value="high">High</option></select></label><label style={label}>Size<select value={layout} onChange={(e) => setLayout(e.target.value as Layout)} style={field}>{Object.entries(dims).map(([k, v]) => <option key={k} value={k}>{v.label}</option>)}</select></label><label style={label}>Base style<select value={style} onChange={(e) => setStyle(e.target.value as Style)} style={field}><option value="clean-product">Clean Campaign</option><option value="social">Social</option><option value="reel-cover">Story / Reel</option><option value="flyer">Flyer</option></select></label></div>{creativeMode !== "product" ? <><div className="photoAdV3__grid"><label style={label}>Identity Lock<select value={identityLock} onChange={(e) => { setIdentityLock(e.target.value as IdentityLock); resetEdit(); }} style={field}><option value="strong">Strong · preserve me closely</option><option value="medium">Medium</option><option value="flexible">Flexible</option></select></label><label style={label}>Hero Priority<select value={heroPriority} onChange={(e) => { setHeroPriority(e.target.value as HeroPriority); resetEdit(); }} style={field}><option value="product">Product Hero</option><option value="shared">Shared Hero · product + me</option><option value="owner">Owner Hero</option></select></label></div><div className="photoAdV3__directionGrid"><label style={label}>Gaze<select value={gaze} onChange={(e) => { setGaze(e.target.value); resetEdit(); }} style={field}>{gazes.map((x) => <option key={x}>{x}</option>)}</select></label><label style={label}>Expression<select value={expression} onChange={(e) => { setExpression(e.target.value); resetEdit(); }} style={field}>{expressions.map((x) => <option key={x}>{x}</option>)}</select></label><label style={label}>Interaction<select value={interaction} onChange={(e) => { setInteraction(e.target.value); resetEdit(); }} style={field}>{interactions.map((x) => <option key={x}>{x}</option>)}</select></label></div><label style={label}>Extra directions<textarea value={directions} onChange={(e) => { setDirections(e.target.value); resetEdit(); }} rows={3} placeholder="Optional manual override." style={{ ...field, resize: "vertical" }} /></label></> : null}<div><strong style={{ color: "#b7cad8", fontSize: 12 }}>Creative refinement</strong><div className="photoAdV3__grid" style={{ marginTop: 8 }}>{(["premium", "bold", "minimal", "lifestyle", "balanced"] as Refinement[]).map((x) => <button key={x} type="button" onClick={() => { setRefinement(x); resetEdit(); }} style={{ ...buttonStyle, background: refinement === x ? "rgba(113,220,255,.18)" : buttonStyle.background }}>{x === "premium" ? "More Premium" : x === "bold" ? "More Bold" : x === "minimal" ? "More Minimal" : x === "lifestyle" ? "More Lifestyle" : "Balanced"}</button>)}</div></div></div></details>
