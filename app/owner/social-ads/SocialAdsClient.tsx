...
export default function SocialAdsClient() {
  ...
  useEffect(() => {
    ...
    const clearStaleOpening = () => setOpening("");
    window.addEventListener("pageshow", clearStaleOpening);
    window.addEventListener("focus", clearStaleOpening);
    return () => {
      window.removeEventListener("pageshow", clearStaleOpening);
      window.removeEventListener("focus", clearStaleOpening);
    };
  }, []);
  ...
  async function openAd(item: CatalogProduct) {
    ...
    try {
      ...
      if (reason instanceof DOMException && reason.name === "AbortError") {
        setError("Opening the ad workspace timed out. You can tap this or another product again.");
      } else {
