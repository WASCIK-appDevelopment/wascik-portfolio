  ...
  return <>
        ...
        {services.map((item) => {
          ...
          return <article key={item.id} style={{ border: "1px solid rgba(113,220,255,.38)", borderRadius: 14, padding: 10, background: "linear-gradient(145deg,rgba(22,111,168,.18),rgba(255,255,255,.03))", color: "#eef8ff" }}>
            <button type="button" onPointerDown={() => beginHold(item)} onPointerUp={() => mediaTap(item)} onPointerCancel={cancelHold} onPointerLeave={cancelHold} onContextMenu={(event) => event.preventDefault()} disabled={Boolean(opening)} aria-label={`${item.title}. Tap to build an ad. Press and hold to replace approved photo.`} style={{ width: "100%", border: 0, padding: 0, background: "transparent", color: "inherit", cursor: opening ? "wait" : "pointer", textAlign: "left", touchAction: "manipulation" }}>
              {media?.url ? <div style={{ width: "100%", height: 118, overflow: "hidden", borderRadius: 10, border: "1px solid rgba(113,220,255,.22)", background: "#07131d" }}><img src={media.url} alt={`${item.title} approved WASCIK media`} style={{ width: "100%", height: "100%", objectFit: "cover", objectPosition: "center", display: "block" }} /></div> : <div style={{ height: 118, overflow: "hidden", borderRadius: 10, border: "1px solid rgba(113,220,255,.18)", background: "radial-gradient(circle at 20% 20%,rgba(113,220,255,.18),rgba(3,12,21,.75))", display: "grid", placeItems: "center", color: "#71dcff", fontSize: 11, fontWeight: 950, letterSpacing: ".12em" }}>WASCIK APPROVED MEDIA</div>}
    ...
    {editing ? <div role="dialog" aria-modal="true" aria-label={`Approved media for ${editing.title}`} onClick={() => !saving && setEditing(null)} style={{ position: "fixed", inset: 0, zIndex: 1300, display: "grid", placeItems: "center", padding: 16, background: "rgba(0,0,0,.78)" }}>
      <section onClick={(event) => event.stopPropagation()} style={{ width: "min(640px,100%)", maxHeight: "82vh", overflowY: "auto", borderRadius: 18, border: "1px solid rgba(113,220,255,.34)", background: "#07131d", padding: 16, color: "#eef8ff" }}>
        ...
        {!loadingPhotos && photos.length ? <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill,minmax(105px,1fr))", gap: 9, marginTop: 14 }}>{photos.map((photo) => <button key={photo.id} type="button" disabled={saving} onClick={() => void assignPhoto(photo)} style={{ border: assignments[editing.id]?.photoId === photo.id ? "2px solid #71dcff" : "1px solid rgba(255,255,255,.12)", borderRadius: 11, padding: 6, background: "rgba(255,255,255,.03)", color: "white", textAlign: "left" }}><div style={{ width: "100%", height: 112, overflow: "hidden", borderRadius: 7, background: "#07131d" }}><img src={photo.url} alt={photo.label} style={{ width: "100%", height: "100%", objectFit: "cover", objectPosition: "center", display: "block" }} /></div><div style={{ marginTop: 5, fontSize: 10, fontWeight: 850, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{photo.label}</div><div style={{ fontSize: 9, color: "#8fa6b6" }}>{photo.category}</div></button>)}</div> : null}
